...
func InitDB(dbType, host, user, password, dbName string, port int, prefix string, logLevel string) (err error) {
	if dbType == "mysql" {
		...
		options := map[string]string{
			"appName":        "security-gateway",
			"connectTimeout": "30000",
		}
		dsn := dameng.BuildUrl(user, password, host, port, options)
		DB, err = gorm.Open(dameng.Open(dsn), &gorm.Config{})
	} else {
		err = fmt.Errorf("不支持的数据库: %s", dbType)
		logrus.Error(err)
		return
	}
	if err != nil {
